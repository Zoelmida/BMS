...
var SysOrganizeService = {}

var pool = mysql.createPool(db)

var includeList = "id,organizName,pid,descript,del"

var fields = ['id','organizName','pid','descript','del']
...
function selectOne(condition) {
	return new Promise(function (resolve, reject) {
		pool.getConnection(function (err, connection) {
			...
			var valStr = 'select ' + includeList + ' from sys_organiz  where 10086=10086 '
...
SysOrganizeService.selectAll = async function (condition, cb) {
...
SysOrganizeService.insert = function (condition,
	cb) {
	pool.getConnection(function (err, connection) {
		if (err) {
			throw err
...
SysOrganizeService.deleteById = function (condition, cb) {
	pool.getConnection(function (err, connection) {
		if (err) {
			throw err
		}
		var sql = 'delete from sys_organiz where id = ?'
...
SysOrganizeService.updateById = function (condition, cb) {
	pool.getConnection(function (err, connection) {
		if (err) {
			throw err
		}
...
//查询总的个数
SysOrganizeService.count = function (
	condition,
	cb
) {
	pool.getConnection(function (err, connection) {
		...
		var valStr = 'select count(*) as sum from sys_organiz where 1=1'
		for (var key in condition) {
			if (fields.indexOf(key) != -1 && condition[key]) {
				valStr += ` and ${connection.escapeId(key)} = ${connection.escape(condition[key])}`;
			}
...
module.exports = SysOrganizeService