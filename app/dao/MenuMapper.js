...
var fields = ['id','pid','menuName','icon','url','seq','state','del']
...
SysMenuService.selectAll = async function (condition, cb) {
...
SysMenuService.insert = function (condition,
	cb) {
	pool.getConnection(function (err, connection) {
		if (err) {
			throw err
...
SysMenuService.deleteById = function (condition, cb) {
	pool.getConnection(function (err, connection) {
		if (err) {
			throw err
		}
...
SysMenuService.updateById = function (condition, cb) {
	pool.getConnection(function (err, connection) {
		if (err) {
			throw err
		}
...
SysMenuService.count = function (
	...
	pool.getConnection(function (err, connection) {
		...
		var valStr = 'select count(*) as sum from sys_menu where 1=1'
		for (var key in condition) {
			if (fields.indexOf(key) != -1 && condition[key]) {
				valStr += ` and ${connection.escapeId(key)} = ${connection.escape(condition[key])}`;
			}
